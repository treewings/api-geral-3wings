...
export default class GetData {
  ...
  public async xmlService(data: IGetData) {
    ...
    if (data.consult === `attendance` || data.consult === `tables`) {
      ...
      body = JSON.parse(body.replace(`0`, `${data.consult == `tables` ? dataCompany.i_code : data.i_code}`))
      ...
      const retAxios = await axios.post(dataConfigApi.url, xml, {
        ...
        .then(async (res) => {
          ...
          try {
            if (data.consult == `attendance` && data.i_code) {
  ...
  public async jsonService(data: IGetData) {
    ...
    if (data.consult === `attendance` || data.consult === `tables`) {
      ...
      let body = dataConfigApi.body
      // trocando o parametro default `0` pelo data.nr_attendance
      body = !body ? null : JSON.parse(body.replace(`0`, `${data.consult == `tables` ? dataCompany.i_code : data.i_code}`))
  ...
  public async internalService(data: IGetData){
    const log = Log('internalService')
